...
float mollify(const Vec3f &n, const Vec3f &wo, const Vec3f &wi, float dist, float radius) {
    float cosMax = 1.0f / glm::sqrt(1.0f + square(radius / dist));
    float sa = 2.0f * Math::Pi * (1.0f - cosMax);
    Vec3f wr = glm::reflect(-wo, n);
    float cosCone = glm::dot(wi, wr);
    return (cosCone >= cosMax) ? 1.0f / (sa * cosCone) : 0;
}
